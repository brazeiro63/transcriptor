...
subject_development_task = initial_tasks.subject_development_task(theologian_agent, bible_verse)  # noqa: E501

# Adicione depuração para verificar a tarefa criada
...
preach_crew = Crew(
    agents=[theologian_agent, preacher_agent, preacher_agent],
    initial_tasks=[
        subject_development_task,
        preach_development_task,
        preach_format_task,
    ],
    verbose=True,
)
...
# Adicione depuração para verificar a tarefa criada
print(f"Tarefa criada: {storytelling_task}")


# cria crew para roteiro do video
video_crew = Crew(
    agents=[video_maker_agent], final_tasks=[storytelling_task], verbose=True
)
